...
const rules = {
	WHITESPACE: /^(?:()(\s+))/,
	ID: /^(?:(\s*)([a-zA-Z]([a-zA-Z\-_0-9]+(:[a-zA-Z\-_0-9]+)?)?))/,
	DOCTYPE: /^(?:(\s*)(!DOCTYPE))/,
	SCRIPT_LITERAL_OPEN: /^(?:(\s*)(<script))/,
	SCRIPT_LITERAL_CLOSE: /^(?:(\s*)(<\/\s*script>))/,
	SCRIPT_BODY: /^((.|\s)*?)(?=<\/\s*script>)/,
	STYLE_LITERAL_OPEN: /^(?:(\s*)(<style))/,
	STYLE_LITERAL_CLOSE: /^(?:(\s*)(<\/\s*style>))/,
	STYLE_BODY: /^((.|\s)*?)(?=<\/\s*style>)/,
	COMMENT_LITERAL: /^(?:(\s*)(<!--(.|\n|\s|\t)*?-->))/,
	COLON: /^(?:(\s*)(:))/,
	SLASH: /^(?:(\s*)(\/))/,
	...
	BRACE_OPEN: /^(?:(\s*)(\{))/,
	BRACE_CLOSE: /^(?:(\s*)(\}))/,
	CONSTS: /^(?:(\s*)(true|false))\b/,
	VAR_NAME: /^(?:(\s*)(\$[a-zA-Z]+([a-zA-Z0-9_-]+)?\b))/,
	NUMBER: /^(?:(\s*)([0-9]+(\.[0-9]+)?\b))/,
	WORD: /^(?:(\s*)([a-zA-Z]+([a-zA-Z0-9_]+)?\b))/,
	MULTIPLICATION: /^(?:(\s*)(\*))/,
	REMINDER: /^(?:(\s*)(%))/,
	MINUS: /^(?:(\s*)(-))/,
	CONCAT: /^(?:(\s*)(\+\+))/,
