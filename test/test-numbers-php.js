...
describe ('PHP number functions', function () {
  it ('num_int', function () {
    return parsePhp('<component>{ num_int(11/3) }</component>').should.eventually.equal('3')
  })

  it ('num_int from string', function () {
    return parsePhp('<component>{ num_int(\'123.456\') }</component>').should.eventually.equal('123')
  })
  ...
  it ('num_round to up', function () {
    return parsePhp('<component>{ num_round(11/3) }</component>').should.eventually.equal('4')
  })

  it ('num_round to down', function () {
    return parsePhp('<component>{ num_round(15/7) }</component>').should.eventually.equal('2')
  })

  it ('num_round half to up', function () {
    return parsePhp('<component>{ num_round(15/2) }</component>').should.eventually.equal('8')
  })
  ...
  it ('num_round negative half to up', function () {
    return parsePhp('<component>{ num_round(-15/2) }</component>').should.eventually.equal('-7')
  })

  it ('num_sqrt', function () {
    return parsePhp('<component>{ num_sqrt(16) }</component>').should.eventually.equal('4')
  })
