...
function appendNode(node) {
  ...
  var isReservedOrComponentName = isTag && (~reservedTags.indexOf(node.name) || ~node.name.indexOf('-'))
  var isLastChideIsFirstChild = isTag && currentNode.name === 'component' && currentNode.firstChild === currentNode.lastChild
  var isLogic = node.type === 'logic-node' || node.type === 'logic'
